...
from fruit30_data import Fruit30Dataset
from fruit30_data import get_train_data as get_train_data_fruit30
from fruit30_data import get_validation_data as get_validation_data_fruit30



class ResNetTrainer:
    def __init__(self, model_path, num_classes: int, epochs: int, train: bool = False):
    ...
    def save_model(self):
        ...
        with open(os.path.join(self.model_path, 'loss_and_accuracy.json'), 'w+') as outfile:
    ...
    def show_loss_and_accuracy(self):
        with open(os.path.join(self.model_path, 'loss_and_accuracy.json'), 'r') as outfile:
...
def get_train_data():
    ...
    train_data_ = DataLoader(train_dataset, batch_size=256, shuffle=True)
...
def get_validation_data():
    ...
    val_data_ = DataLoader(val_dataset_, batch_size=256, shuffle=False)

    return val_data_, val_dataset_.classes


if __name__ == '__main__':
    train_data, labels = get_train_data_fruit30()
    val_data, val_labels = get_validation_data_fruit30()
    print(labels)

    trainer = ResNetTrainer('./data/fruit30', num_classes=len(labels), epochs=50, train=True)

...
def get_train_data():
    ...
    train_data_ = DataLoader(train_dataset, batch_size=300, shuffle=True)
...
def get_validation_data():
    ...
    val_data_ = DataLoader(val_dataset_, batch_size=300, shuffle=False)

    return val_data_, val_dataset_.classes

class VGG16NetTrainer:
    def __init__(self, num_classes: int, epochs: int, train: bool = False):
        ...
        if self.train_mode:
            self.model = VGG16Net(num_classes)
...
if __name__ == '__main__':
    ...
    trainer = VGG16NetTrainer(num_classes=len(labels), epochs=20, train=True)
    trainer.train(train_data, val_data)
    trainer.save_model()

    VGG16NetTrainer.show_loss_and_accuracy()

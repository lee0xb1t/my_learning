...
/**
 * @brief  绘制标准单色 BMP 图像（1 bit per pixel）
 * @param  x       : 起始X坐标
 * @param  y       : 起始Y坐标
 * @param  bmp     : 指向完整 BMP 文件数据的指针（包含文件头）
 * @param  color   : 绘制颜色
 * @note   BMP格式必须为单色（BI_RGB压缩），不支持压缩格式
 */
void ssd1306_DrawBMP(uint8_t x, uint8_t y, const unsigned char* bmp, SSD1306_COLOR color)
{
    // BMP文件头结构（标准14字节）
    if (bmp[0] != 'B' || bmp[1] != 'M') {
        return;  // 不是BMP文件
    ...
    // 读取数据偏移量（从文件头偏移0x0A开始的4字节）
    uint32_t dataOffset = *(uint32_t*)&bmp[0x0A];

    // 读取DIB头，获取图像尺寸（偏移0x12开始）
    int32_t width  = *(int32_t*)&bmp[0x12];   // 宽度（有符号，通常为正）
    int32_t height = *(int32_t*)&bmp[0x16];   // 高度（正数 = 从下到上存储）

    uint16_t bitCount = *(uint16_t*)&bmp[0x1C]; // 位深度（必须为1）

    // 验证格式
    if (bitCount != 1 || width <= 0 || height == 0) {
        return;
    }

    // BMP单色图像的调色板大小（2个颜色项，共8字节）
    uint32_t paletteSize = (bitCount == 1) ? 8 : 0;

    // 计算每行字节数（BMP每行必须4字节对齐）
    uint32_t rowSize = ((width + 31) / 32) * 4;  // 等同于 ((w+7)/8 + 3) & ~3
    ...
    for (uint8_t j = 0; j < height; j++) {
        int32_t bmpRow = startRow + step * j;          // BMP中实际的行号
        uint32_t rowOffset = dataOffset + bmpRow * rowSize;  // 该行数据在BMP中的偏移
        ...
        for (uint16_t i = 0; i < width; i++) {
            // 从BMP中读取字节（每8个像素读一次）
            if ((i & 7) == 0) {
                uint32_t byteOffset = rowOffset + (i / 8);
                currentByte = bmp[byteOffset];

                // BMP的MSB在左，与oled_drawBitmap保持一致
                // 如果BMP格式需要反转，可启用下一行：
                // currentByte = reverseBits(currentByte);
            }
